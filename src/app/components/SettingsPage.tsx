...
function ReportSyncButtons({
  ...
  const handlePull = async () => {
    if (!uid) {
      setResult({ ok: false, message: "로그인이 필요합니다. (설정 > 로그인)" });
...
export function SettingsPage() {
  ...
  const handleExportPdfZipToStorage = async () => {
    ...
    try {
      ...
      if (!ok || !blob) {
        setExportStatus({ type: "pdfzip", ok: false, message: error || "PDF 생성 실패" });
      } else {
        ...
        setExportStatus({
          type: "pdfzip",
          ok: result.ok,
          message: result.ok ? "PDF(ZIP)을 로컬에 저장했습니다." : (result.error || "로그인??"),
        });
      }
    } finally {
    ...
    try {
      ...
      if (!ok || !blob) {
        ...
        setExportStatus({
          type: "pdfzip",
          ok: result.ok,
          message: result.ok ? "PDF(ZIP)을 Google Drive에 저장했습니다.." : (result.error || "로그인?"),
        });
      }
    } finally {
      setExportPdfLoading(false);
        ...
        <div className="bg-white/5 border border-white/8 rounded-[10px] overflow-hidden">
          ...
          <div className="pt-2">
            <div style={{ fontSize: 12 }} className="text-white/40 mb-1">
              {apiStatus.gpt === "ok" && apiStatus.gemini === "ok" && apiStatus.anthropic === "ok" ? "로그인" : "로그인?"} :
            </div>
            <div
                ...
                <div className="fixed bottom-24 left-4 right-4 max-w-[430px] mx-auto rounded-[10px] border border-white/10 bg-[#12121a] shadow-xl z-[101] overflow-hidden">
                  ...
                  >
                    ...
                    <span className="text-white/90">
                      {exportPdfLoading ? "PDF 생성 중…" : "PDF(ZIP) 로컬 저장"}
                    </span>
                  </button>
                  <button
                    type="button"
...
<section className="mb-4">
      ...
      {/* ?? 내보내기 (lightweight-charts attributionLogo ?데이터 동기화로그인) */}
      <section className="mb-4">
