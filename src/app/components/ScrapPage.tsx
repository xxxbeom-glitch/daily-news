...
export function ScrapPage() {
  ...
  const isUnderSettings = location.pathname === "/settings/scrap";
  ...
  const backLink = isUnderSettings ? (
    <Link
      to="/settings"
      className="flex items-center gap-2 text-white/70 hover:text-white py-3"
      style={{ fontSize: 14 }}
    >
      <ArrowLeft size={18} />
      설정으로
