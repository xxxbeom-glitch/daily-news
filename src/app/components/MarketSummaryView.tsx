    ...
    return (
      <div className="bg-white/5 border border-white/8 rounded-[10px] overflow-hidden">
        ...
        <div className="px-5 py-0">
          ...
          <BlockTitle emoji="📈">{data.stockMoversLabel} 상승·하락 TOP3</BlockTitle>
          <div className="mt-[14px] space-y-[9px]">
            <div style={{ fontSize: 14, fontWeight: 600, ...lineStyle }} className="text-white">상승 TOP 3</div>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, ...lineStyle }} className="text-white mt-[18px]">하락 TOP 3</div>
          ...
          <BlockTitle emoji="💻">{data.bigTechLabel}</BlockTitle>
          <div className="mt-[14px] space-y-[9px]">
            {data.bigTechIssues.map((item: IssueItem, i: number) => {
              const m = item.title.match(/^(.+?)\(([A-Za-z0-9]+)\)$/);
              const company = m ? m[1] : item.title;
              const ticker = m ? m[2] : null;
              return (
                <div key={i}>
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span style={{ fontSize: 14, fontWeight: 500, ...lineStyle }} className="text-white/90">{company}</span>
                    {ticker && <span style={{ fontSize: 14 }} className="text-white/40">({ticker})</span>}
                    {item.changeRate && (
                      <span style={{ fontSize: 14 }} className={item.changeRate.startsWith("+") ? "text-emerald-400" : "text-red-400"}>
                        {item.changeRate}
                      </span>
                    )}
                  </div>
          ...
          {data.bigTechSources.length > 0 && (
            <div style={{ fontSize: 13, ...lineStyle }} className="text-white/40 mt-[16px] mb-[22px]">
              출처: {data.bigTechSources.map((s) => s.outlet).join(", ")}
            </div>
          )}
  ...
  return (
    <div className="bg-white/5 border border-white/8 rounded-[10px] overflow-hidden">
      ...
      <div className="px-5 py-0">
        ...
        {data.kosdaqMovers && (
          <>
            <BlockTitle emoji="📈">코스닥 기업 상승 및 하락 TOP3</BlockTitle>
            <div className="mt-[14px] space-y-[9px]">
              <div style={{ fontSize: 14, fontWeight: 600, ...lineStyle }} className="text-white">상승 TOP 3</div>
              {data.kosdaqMovers.up.map((m, i) => (
                <div key={i}>
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span style={{ fontSize: 14, fontWeight: 500, ...lineStyle }} className="text-white/90">{m.name}</span>
                    <span style={{ fontSize: 14 }} className="text-white/40">({m.ticker})</span>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, ...lineStyle }} className="text-white mt-[18px]">하락 TOP 3</div>
              {data.kosdaqMovers.down.map((m, i) => (
                <div key={i}>
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span style={{ fontSize: 14, fontWeight: 500, ...lineStyle }} className="text-white/90">{m.name}</span>
                    <span style={{ fontSize: 14 }} className="text-white/40">({m.ticker})</span>
            ...
            {data.kosdaqMovers.sources.length > 0 && (
              <div style={{ fontSize: 13, ...lineStyle }} className="text-white/40 mt-[16px] mb-[22px]">
                출처: {data.kosdaqMovers.sources.map((s) => s.outlet).join(", ")}
              </div>
            )}
          </>
        )}
        ...
        {data.kospiMovers && (
          <>
            <BlockTitle emoji="📈">코스피 기업 상승 및 하락 TOP3</BlockTitle>
            <div className="mt-[14px] space-y-[9px]">
              <div style={{ fontSize: 14, fontWeight: 600, ...lineStyle }} className="text-white">상승 TOP 3</div>
              {data.kospiMovers.up.map((m, i) => (
                <div key={i}>
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span style={{ fontSize: 14, fontWeight: 500, ...lineStyle }} className="text-white/90">{m.name}</span>
                    <span style={{ fontSize: 14 }} className="text-white/40">({m.ticker})</span>
            ...
            {data.kospiMovers.sources.length > 0 && (
              <div style={{ fontSize: 13, ...lineStyle }} className="text-white/40 mt-[16px] mb-[22px]">
                출처: {data.kospiMovers.sources.map((s) => s.outlet).join(", ")}
              </div>
            )}
          </>
        )}
