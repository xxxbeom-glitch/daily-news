...
4. 기사 내용: 이해하기 쉽도록 최소 3줄 이상으로 요약. 명사형 종결(-음, -기, -함, -됨). 개조식. 문두에 하이픈(-), 불릿(•), 번호(1. 2.) 등 리스트 마커 사용 금지.
5. 한자 한글화: 기사 제목·내용에 한자가 있으면 문맥에 맞는 한글로 치환하세요. 예: 美→미국, 中→중국, 日→일본, 韓→한국, 北→북한, 歐→유럽, 獨→독일, 英→영국, 中東→중동. 문맥에 따라 적절한 한글 의미로 변환.`;
...
- PDF 문서: 페이지별·기사별로 개별 추출. [PDF]로 시작하는 텍스트는 PDF에서 추출된 내용임.

출력 형식:
- 기사제목: 원문 그대로 (예: "팔란티어 (PLTR) +14.16%")
- 기사내용: 원문에서 이해되기 쉽게 최소 3줄 이상 요약. 한자는 문맥에 맞는 한글로 치환 (美→미국, 中→중국 등)
