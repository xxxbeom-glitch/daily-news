...
${includeTotalAssessment
    ? isInternational
      ? `- totalAssessment: [필수] 핵심 이슈 3~4가지로 나누어 작성. 각 항목당 2줄 내외. 개조식·명사형 종결(-음, -기, -함, -됨). 줄바꿈으로 항목 구분. 서술형 긴 문단 금지. 수집된 기사 기반으로만 작성.`
...
function ensureIssueItems(items: { title?: string; body?: string; changeRate?: string; section?: string }[]): IssueItem[] {
  return items?.map((i) => ({
    title: i?.title ?? "",
    body: i?.body ?? "",
...
/** 테스트2(한국경제 헤드라인)용 시스템 프롬프트 */
const MORNING_HEADLINE_SYSTEM_PROMPT = `역할: 귀하는 한국경제 헤드라인 정리 전문가입니다.
...
- 같은 기사에서 나온 항목은 title을 동일하게, body만 각 항목의 핵심 내용으로 다르게.
- 이미지 1장에 기사 여러 개: 각 기사별로 위와 같이 항목 분리.
- PDF: [PDF] 텍스트는 PDF 추출 내용임. 기사별·항목별 분리.
- 섹션: 각 항목마다 section을 반드시 지정. (경제, 증권, 정치, 사회, IT/과학, 부동산, 국제, 기타 중 하나)

출력 형식:
- title: 기사 제목 원문 그대로
- body: 해당 항목의 핵심 내용만 1~2문장. 개조식·명사형 종결.
- section: 기사 성격에 맞는 섹션명 (경제|증권|정치|사회|IT/과학|부동산|국제|기타)

반드시 아래 JSON 형식으로만 응답하고, 다른 텍스트는 포함하지 마세요.
{
  "date": "YYYY. MM. DD (요)",
  "regionLabel": "한국경제 헤드라인",
  "keyIssues": [
    { "title": "기사제목", "body": "항목1 핵심 내용", "section": "경제" },
    { "title": "기사제목", "body": "항목2 핵심 내용", "section": "증권" }
  ]
}
keyIssues만 채우세요. section은 반드시 포함하세요. 반드시 유효한 JSON만 출력하세요.`;

/** 테스트4: Global Market Daily PDF 전용 시스템 프롬프트 */
const GLOBAL_MARKET_DAILY_SYSTEM_PROMPT = `역할: 귀하는 Global Market Daily·Insight PDF 분석 전문가입니다.
...
4. **핵심 10개(keyIssuesTop10)**: 뉴스브리프(News Brief) 내용 중 시장 영향도·글로벌 이슈·핵심 수치 기준으로 가장 중요한 10개 항목만 선별. 마켓 하이라이트는 제외. 각 항목 title만 출력. 1~2줄 분량으로 핵심 내용 포함. body 생략.
...
6. keyIssuesTop10: 뉴스브리프(News Brief)에서만 시장·경제·지정학적 중요도가 높은 10개만 선별. 마켓 하이라이트는 포함하지 않음. 각 항목 title만 출력. 1~2줄 분량으로 핵심 내용 포함. body 생략.
...
[PDF 1], [PDF 2] 두 PDF가 있으면 통합하여 하나의 JSON으로 출력하세요.
...
4. **핵심 10개(keyIssuesTop10)** - 뉴스브리프(News Brief)에서만 시장·경제·지정학적 중요도가 높은 10개만 선별. 마켓 하이라이트 제외. 각 항목 title만. 1~2줄 분량으로 핵심 내용 포함. body 생략.
...
반드시 아래 JSON 형식으로만 응답하세요.
{
  ...
  "indices": [
    { "name": "S&P500", "value": "종가", "change": "+0.78%", "isUp": true },
    { "name": "나스닥", "value": "종가", "change": "+1.29%", "isUp": true },
    { "name": "다우존스", "value": "종가", "change": "+0.49%", "isUp": true },
    { "name": "금", "value": "종가", "change": "+0.21%", "isUp": true },
    { "name": "은", "value": "종가", "change": "-0.35%", "isUp": false },
    { "name": "구리", "value": "종가", "change": "-3.10%", "isUp": false },
    { "name": "WTI", "value": "종가", "change": "+0.13%", "isUp": true }
  ],
  "sectorEtf": [
    { "name": "SPY", "value": "종가", "change": "+0.76%", "isUp": true },
  ...
  "keyIssuesTop10": [
    { "title": "뉴욕증시, 유가 폭등·고용 충격에 이틀 연속 하락 마감. 국제 유가 원유 공급 불안으로 미국 산업 전반 충격." },
    { "title": "미국 노동부 2월 비농업 부문 고용 9만2천명 감소 발표." }
  ]
}
반드시 유효한 JSON만 출력하세요.`;
...
/**
 * 테스트4: Global Market Daily PDF 전용 분석
 */
export async function generateGlobalMarketDailyFromPdf(
  pdfText: string,
  opts?: GenerateFromUploadedOptions
): Promise<MarketSummaryData> {
...
function parseGlobalMarketDaily(jsonStr: string): MarketSummaryData {
  ...
  result.sectorEtf = toIndexData((parsed.sectorEtf as unknown[]) ?? []);
  result.currencies = [];
  result.commodities = [];
  const rawIssues = (parsed.keyIssues as { title?: string; body?: string }[]) ?? [];
  result.keyIssues = ensureIssueItems(rawIssues).map((item) => ({
    ...item,
    title: (item.title ?? "").replace(/^\s*■\s*/, "").trim(),
  }));
  const rawTop10 = (parsed.keyIssuesTop10 as { title?: string; body?: string }[]) ?? [];
  result.keyIssuesTop10 = ensureIssueItems(rawTop10).slice(0, 10).map((item) => ({
    ...item,
    title: (item.title ?? "").replace(/^\s*■\s*/, "").trim(),
  }));
  return result;
...
/** 한국경제 헤드라인 전용 파싱 - keyIssues만 사용, 나머지 제거 */
function parseMorningHeadline(jsonStr: string): MarketSummaryData {
  ...
  const keyIssues = ensureIssueItems((parsed.keyIssues as { title?: string; body?: string; section?: string }[]) ?? []);

  return {
    date: (parsed.date as string) ?? dateStr,
    regionLabel: (parsed.regionLabel as string) ?? "한국경제 헤드라인",
    indices: [],
    indicesSources: [],
    keyIssues,
    keyIssuesSources: [],
