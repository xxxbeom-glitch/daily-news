...
function sessionToPrintableHtml(session: ArchiveSession): string {
  ...
  const isIntl = data.regionLabel.includes("해외") || data.regionLabel.includes("글로벌");
  ...
  return `
    <div style="width:${PDF_WIDTH}px;padding:28px;font-family:'Malgun Gothic',sans-serif;background:#fff;color:#111;">
      <div style="border-bottom:1px dashed #ddd;padding-bottom:20px;margin-bottom:20px;">
        ...
        <div style="font-size:18px;font-weight:700;${lineStyle}">${escapeHtml(isIntl ? "해외 시황 요약" : "한국 시장 뉴스")}</div>
        <div style="font-size:11px;color:#888;margin-top:6px;">Generated by ${aiModel === "gemini" ? "Gemini AI" : aiModel === "claude" ? "Claude" : "ChatGPT"}</div>
      </div>
      <div style="margin-top:22px;padding-top:22px;border-top:1px dashed #ddd;">
        <div style="font-size:16px;font-weight:700;${lineStyle}">📝 ${isIntl ? "오늘의 시황 총평" : "총평"}</div>
